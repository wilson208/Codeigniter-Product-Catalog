...
class user extends My_Controller{
    ...
    function add(){
        $id = $this->user->newUser('id', 'Title', 'Forename', 'Surname', 'Email', 'Address 1', 'Address 2', 'Town', 'Postcode', 'County', 'Country', 'Phone');
	...
	$this->form_validation->set_rules('phone', 'Phone', 'trim|required|xss_clean|max_length[16]');
        ...
        if ($this->form_validation->run() != FALSE){
            ...
            $this->user->newUser($id, $title, $forename, $surname, $email, $address1, $address2, $town, $postcode, $county, $country, $phone);
            redirect(admin_url('user/single?id=') . $id, 'refresh');
	}else{
